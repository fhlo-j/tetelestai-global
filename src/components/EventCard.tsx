...
const EventCard = ({ id, title, date, time, location, imageUrl, featured = false, description }: EventCardProps) => {
  return (
      ...
      <div className="p-4">
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <Calendar size={14} className="text-divine" />
            <span>{date} • {time}</span>
          </div>
          
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <MapPin size={14} className="text-divine" />
            <span>{location}</span>
          </div>
        </div>
